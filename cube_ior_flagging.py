...
def write_statistics_file(statsDict, conf):
    ...
    legendList = ["chanNo", "frequency [MHz]",  "rmsStokesI [uJy/beam]", "rmsStokesV [uJy/beam]", "flagged", "maxStokesI [uJy/beam]"]
...
def plot_all(xData, yData, yDataFit, std, outlierIndexSet, iteration, conf):
...
def get_outlierIndex_and_fitStats_dict(xData, yData, conf):
    resultsDict = {}
    ...
    while outlierSwitch:
        ...
        if CREATE_ITERATION_PLOTS:
            plot_all(resultsDict['xData'], resultsDict['yData'], get_yDataFit(resultsDict['xData'], a, b, c), std, outlierIndexSet, iteration, conf)
            iteration += 1
...
def main():
    ...
    resultsDict = get_outlierIndex_and_fitStats_dict(xData, yData, conf)
